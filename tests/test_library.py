...
class PostgresLibraryTests(unittest.TestCase):
    ...
    def test_filter_tracks_with_advanced_meta_search(self):
        """Test filtering of tracks."""
        nd.library.reset(force=True)
        inserted_track1 = nd.library.add_track(
            file_path="tests/assets/test.mp3",
        )
        inserted_track1.set_meta({
            "artist": "Michael Jackson",
            "title": "Thriller",
        })
        example_data = nd.library.filter_tracks_by_meta(
            search_meta={"artist": ["jackson"]},
        )
        ...
        self.assertEqual(len(example_data), 1)
        example_data = nd.library.filter_tracks_by_meta(
            search_meta={"artist": ["Gorillaz"]},
        )
        self.assertEqual(len(example_data), 0)
        example_data = nd.library.filter_tracks_by_meta(
            search_meta={"": ["jackson", "michael"]},
        )
        self.assertEqual(len(example_data), 1)
        example_data = nd.library.filter_tracks_by_meta(
            search_meta={"": ["artist"]},
    ...
    def test_filter_tracks_returns_filtered_tracks(self):
        ...
        inserted_track1.set_meta({"test_meta_key": "test_meta_value"})
